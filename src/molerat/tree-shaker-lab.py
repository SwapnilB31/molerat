...
def get_local_modules(path: str):
    ...
    for directory in directories:
        files = [
            dirent.name
            for dirent in os.scandir(os.path.join(path, directory))
            if dirent.is_file()
        ]
...
class LocalImportVisitor(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        for alias in node.names:
            ...
            if (directory_name in self.local_modules and node.level == 0) or (
                directory_name in self.lvl2_local_modules and node.level == 2
            ):
                functional_identifier = alias.asname if alias.asname else alias.name
                ...
                if functional_identifier != "*":
                    ...
                    print(
                        f"Wildcard imports for module_path = {file_path} are: ", imports
                    )
...
if __name__ == "__main__":
    liv = LocalImportVisitor(
        os.path.join(os.getcwd(), "module_a/mod.py"), "module_a/mod.py"
    )
    # liv.visit()
